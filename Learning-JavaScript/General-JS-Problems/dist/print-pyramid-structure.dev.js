...
function pyramid(totalRows) {
  for (var row = 0; row < totalRows; row++) {
    var line = ''; // loop for first column-1 where I will print a single space

    for (var col1 = 0; col1 < totalRows - row; col1++) {
      line += " ";
...
pyramid(5); // SOLUTION-2 - THIS TECHINIQUE SHOULD BE IMPLEMENTED FOR FUTURE PATTERN BUILDING >>
...
printPyramid = function printPyramid(totalRows) {
  for (var row = 0; row < totalRows; row++) {
    var line = '';
    ...
    for (var col2 = 0; col2 <= 2 * row + 1; col2++) {
      line = line + "*";
    }
