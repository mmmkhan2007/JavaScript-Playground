...
var isMatchingBrackets = function isMatchingBrackets(str) {
  var stack = [];
  var map = {
    '(': ')',
    '[': ']',
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (str[i] === '(' || str[i] === '{' || str[i] === '[') {
      stack.push(str[i]);
        ...
        var last = stack.pop(); //If the popped element from the stack, which is the last opening brace doesn’t match the corresponding closing brace in the map, then return false
...
var isParenthesisMatching = function isParenthesisMatching(str) {
  var stack = [];
  var open = {
    '{': '}',
    '[': ']',
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (open[_char]) {
      stack.push(_char);
    } else if (closed[_char]) {
      if (open[stack.pop()] !== _char) return false;
    }
  }
...
var isBalancedParenthesis = function isBalancedParenthesis(str) {
  return !str.split('').reduce(function (uptoPrevChar, thisChar) {
...
function isBalanced(_ref) {
  var _ref2 = _toArray(_ref),
      str = _ref2.slice(0);

  return str.reduce(function (uptoPrevChar, thisChar) {
...
isBalancedBraces = function isBalancedBraces(str) {
  var count = 0;
  ...
  try {
    for (var _iterator = str[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var i = _step.value;
      if (i === "{") count++;else if (i === "}") count--; // Noting from the first test-case it looks like that if a closing curly brace is first encountered, then my function should return 'false'. So as soon as I have a negative value of count, I should return false.
