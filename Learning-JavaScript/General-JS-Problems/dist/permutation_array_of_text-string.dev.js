...
function findPermutation(inputStr) {
  ...
  for (var i = 0; i < inputStr.length; i++) {
    var firstChar = inputStr[i]; // This char will be fixed for the below iterations. i.e. I will not touch this char, as it will be kept as an achor and shuffle the rest of the chars. And prepend this char, to all permutations of the string without this char in it.

    var charsLeftOver = inputStr.substring(0, i) + inputStr.substring(i + 1); // Note, in the both the use of the substring() method, I am not touching the character at i-th index-position

    var innerPermutations = findPermutation(charsLeftOver); //So each innerPermutations will be an array.
