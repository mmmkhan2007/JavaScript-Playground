...
var swap = function swap(str, idx1, idx2) {
    var arr = str.split(""),
        temp = arr[idx1];
    arr[idx1] = arr[idx2];
    arr[idx2] = temp;
...
var getPivot = function getPivot(str, compare) {
    for (var i = str.length; str > 0; i--) {
        if (str[i] > compare) return i;
    }

    return str.length - 1;
};

var nextBigger = function nextBigger(str) {
    str = str.toString();

    for (var i = str.length - 1; i >= 0; i--) {
        if (str[i] > str[i - 1]) {
            str = swap(str, i - 1, getPivot(str, str[i - 1]));
            return parseInt(str.substring(0, i) + str.substring(i).split("").map(function(a) {
                return parseInt(a);
            }).sort(function(a, b) {
                return a - b;
            }).join(""));
